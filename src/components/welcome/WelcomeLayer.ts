...
const colorPalettes = tailwindColors500.map((color, index) => {
  const nextColor = tailwindColors500[(index + 1) % tailwindColors500.length] ?? color;
...
export class WelcomeLayer {
  ...
  /** Public cleanup when Vue (or parent) unmounts before the layer completes. */
  destroy() {
    ...
    this.kwamiContainer = null;
    this.kwami = null;
    this.onCompleteCallback = null;
    this.onExitStartCallback = null;
  }
  ...
  private preloadAudio() {
    ...
    this.audioElement = new Audio('/welcome.mp3');
  ...
  private async initWelcomeKwami() {
    ...
    try {
      ...
      const blob = this.kwami.avatar.getBlob();
      if (!blob) {
        console.warn('Welcome Kwami: no blob renderer');
        this.hideLoader();
        return;
      }
      ...
      this.kwami.avatar.setScale(initialScale);
      this.kwami.avatar.setWireframe(false);
      
      // FORCE set colors explicitly after initialization - section 0 white/gray/black gradient
      this.kwami.avatar.setColors('#ffffff', '#808080', '#000000');
      
      // Start FULLY SPHERICAL (no spikes)
      this.kwami.avatar.setSpikes(0, 0, 0);
      blob.setTime(3, 6, 3);
  ...
  private animateBlobToFinalState() {
    const blobRef = this.kwami?.avatar.getBlob();
    if (!blobRef) return;
    const isMobile = window.innerWidth <= 768;
    ...
    this.scrollHandler = () => {
      ...
      if (section !== this.currentSection) {
        ...
        if (section === 0) {
          ...
          if (palette) {
            this.kwami.avatar.setColors('#ffffff', palette.accent, '#000000');
          }
        }
      }
    };
  ...
  private async handleClick() {
    ...
    if (kwamiContainer && svg && this.kwami) {
      ...
      const blobRef = this.kwami.avatar.getBlob();
      if (!blobRef) return;
      const blobMesh = blobRef.getMesh();
