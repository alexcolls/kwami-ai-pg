...
const { fetchSTTInferenceModels, fetchSTTPluginModels, sttInferenceModels, sttPluginModels, isLoading } = useModelsApi();
...
const pluginModelsByProvider = computed(() => {
  if (!sttPluginModels.value?.providers) return {};
  return sttPluginModels.value.providers;
});
...
function getProviderIcon(provider: string): string {
  const icons: Record<string, string> = {
    ...
    elevenlabs: 'ph:microphone-duotone',
...
function selectModel(modelId: string, provider: string) {
  ...
  if (isConnected.value && kwami.value) {
    ...
    console.log('👂 STT updated live:', { provider, model: modelId });
  }
}
...
<template>
  <div class="panel-inner">
    <div class="panel-header">
      <iconify-icon icon="ph:ear-duotone" class="panel-icon"></iconify-icon>
